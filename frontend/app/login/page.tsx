...
import { handleLineLoginError, isLineLoginAvailable } from "@/lib/lineAuth"
...
function LoginForm() {
    ...
    if (errorParam) {
      ...
      if (errorParam === 'OAuthSignin') {
        if (providerParam === 'line') {
          errorMsg = messageParam || 'เกิดข้อผิดพลาดในการเชื่อมต่อกับ LINE กรุณาลองใหม่อีกครั้ง';
        } else {
  ...
  const handleLineLogin = async () => {
    ...
    try {
      ...
      const errorMsg = handleLineLoginError(error);
      setErrorMessage(errorMsg);
      setIsLoading(false);
              ...
              <div className="bg-white rounded-lg p-6 shadow-xl">
                <div className="flex items-center space-x-3">
                  ...
                  <p className="text-gray-700 font-medium">
                    กำลังเข้าสู่ระบบด้วย {loginProvider === 'line' ? 'LINE' : 'Google'}...
                  </p>
                </div>
              </div>
            </div>
